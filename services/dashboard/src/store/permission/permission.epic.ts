...
import { PermissionActionTypes } from './permission.state'
...
const getPermissions: Epic = (action$) =>
  action$.pipe(
    ofType(PermissionActionTypes.LIST_ALL_REQUEST),
    debounceTime(0),
    ...
    mergeMap(() => {
      ...
      return apiSvc.request({ path: '/user?relations=roleMappings,roleMappings.role,roleMappings.institution', requireAuthType: session?.type }).pipe(
        map(({ response }) => {
          return {
            type: PermissionActionTypes.LIST_ALL_SUCCESS,
            payload: response
          }
        }),
        catchError((err) =>
          of({
            type: PermissionActionTypes.LIST_ALL_FAILURE,
            payload: err,
          }),
        ),
      )
...
const getPermissionsFailed: Epic = (action$) =>
  action$.pipe(
    ofType(PermissionActionTypes.LIST_ALL_FAILURE),
    map(() => enqueueSnackbarAction({ variant: 'error', message: 'Error getting users.', key: 'REQUEST_ERROR' })),
  )

export const usersEpics = [getPermissions, getPermissionsFailed]
