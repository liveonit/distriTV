...
import { IUserActionTypes } from '../models/IUsersState'

const refreshToken$ = defer(() => checkOrRefreshToken())

const getUsers: Epic = (action$) =>
  action$.pipe(
    ofType(IUserActionTypes.LIST_ALL_REQUEST),
    debounceTime(0),
    ...
    mergeMap(() => {
      ...
      return apiSvc.request({ path: '/user', requireAuthType: session?.type }).pipe(
        ...
        catchError((err) =>
          of({
            type: IUserActionTypes.LIST_ALL_FAILURE,
            payload: err,
          }),
        ),
      )
...
const getUsersFailed: Epic = (action$) =>
  action$.pipe(
    ofType(IUserActionTypes.LIST_ALL_FAILURE),
