  ...
  action$.pipe(
    ...
    mergeMap(({payload}) => {
      ...
      return apiSvc.request({
        path: `/alert?relations=television,label${payload?.query ? `&${payload.query}` : ''}`,
...
const deleteAlert: Epic = (action$) =>
  action$.pipe(
    ...
    mergeMap(({ payload }) => {
      const { session } = storage.get<SessionT>('session') || {}
      return apiSvc.request({ method: 'DELETE', path: `/alert/${payload.id}`, requireAuthType: session?.type}).pipe(
        mergeMap(({ response }) => {
          return of({
